...
async function occupationMatrix(db,title){
  ...
  let q=await db.from('esco_occupations').select('conceptUri,preferredLabel,altLabels,iscoGroup').ilike('preferredLabel','%'+title+'%').limit(1);
  let occ=q.data?.[0];
  if(!occ){q=await db.from('esco_occupations').select('conceptUri,preferredLabel,altLabels,iscoGroup').ilike('altLabels','%'+title+'%').limit(1);occ=q.data?.[0];}
...
async function enrichRow(db,r,p,workerMode){
  ...
  const vac=vq.data||{};
  let evaluatee=null;
  if(workerMode)evaluatee=vac.employer_id||null;
  else{const wq=await db.from('worker_profiles').select('id,user_id,profession').eq('id',r.worker_profile_id).maybeSingle();if(wq.data?.user_id){const pq=await db.from('profiles').select('id').eq('auth_user_id',wq.data.user_id).maybeSingle();evaluatee=pq.data?.id||null;}}
...
function evaluationCard(r){
 const m=r.matrix||{};const cs=Array.isArray(m.criteria)?m.criteria:[];
 const criteriaHtml=cs.map((c,i)=>{const source=c.source==='ESCO'?'مهارة مهنية وفق ESCO':'معيار مهني عام';const relation=c.relation==='essential'?'أساسية':(c.relation==='optional'?'اختيارية':'عامة');return '<div style="margin:10px 0;padding:11px;border:1px solid rgba(128,128,128,.25);border-radius:10px"><strong>'+esc(c.label_ar||c.code)+'</strong><small style="display:block;opacity:.78;margin-top:3px">'+source+' · '+relation+' · وزن: '+Number(c.weight||1).toFixed(1)+'</small>'+select('c_'+r.id+'_'+i,r.id+':c:'+i)+'</div>';}).join('');
 return '<div class="card" style="margin:12px 0;padding:16px"><h4>التقييم المهني: '+esc(r.vacancy.title||r.vacancy.category||'الفرصة')+'</h4><p>هذا تقييم مهني مرتبط بالمهنة نفسها. تُستمد المعايير المهنية من مهارات ESCO المرتبطة بالمهنة، ثم تُضاف طبقة محدودة من المعايير المهنية العامة. النتيجة مؤشر مهني قابل للتفسير وليست حكماً آلياً على أهلية الشخص.</p><div style="padding:10px;border-radius:10px;background:rgba(128,128,128,.08);margin-bottom:12px"><strong>المهنة:</strong> '+esc(m.occupation_label||r.vacancy.title||r.vacancy.category||'غير محددة')+'<br><strong>مجموعة ISCO:</strong> '+esc(m.isco_group||'غير محددة')+'<br><strong>إطار التقييم:</strong> '+esc(m.framework||'ESCO-aligned')+'</div><h4>معايير التقييم</h4>'+criteriaHtml+'<div style="margin-top:14px"><label><strong>ملاحظة مهنية</strong></label><textarea data-note="'+r.id+'" rows="3" style="width:100%;margin-top:6px" placeholder="اكتب ملاحظة مرتبطة بالأداء الفعلي"></textarea></div><button class="primary-btn" data-eval="'+r.id+'" style="width:100%;margin-top:12px">إرسال التقييم المهني</button></div>';
}
async function render(){
 const db=s();if(!db)return;const p=await me();if(!p)return;let rows=[],workerMode=p.role==='worker';const u=(await db.auth.getSession()).data.session?.user;
 ...
 const ids=enriched.map(r=>r.id);const existing=await db.from('professional_evaluations').select('match_request_id,evaluator_profile_id').in('match_request_id',ids);const done=new Set((existing.data||[]).filter(x=>x.evaluator_profile_id===p.id).map(x=>x.match_request_id));box.innerHTML='<h3>⭐ التقييم المهني</h3><p>التقييم الآن مبني على المهنة الفعلية ومهاراتها وفق ESCO، مع أوزان واضحة، بدل قائمة عامة موحدة.</p>'+enriched.map(r=>done.has(r.id)?'<div class="card" style="margin:10px 0"><strong>تم إرسال تقييمك</strong><p>تم حفظ مساهمتك في السجل المهني.</p></div>':evaluationCard(r)).join('');
 box.querySelectorAll('[data-eval]').forEach(b=>b.onclick=async()=>{const id=b.dataset.eval,r=enriched.find(x=>x.id===id),m=r?.matrix||{},cs=Array.isArray(m.criteria)?m.criteria:[];if(!r?.evaluatee){toast('تعذر تحديد الطرف الذي سيُقيّم.');return;}if(!cs.length){toast('تعذر تحميل معايير المهنة.');return;}const scores=cs.map((c,i)=>({code:c.code,label_ar:c.label_ar,weight:Number(c.weight||1),score:Number(box.querySelector('[data-score="'+id+':c:'+i+'"]').value),source:c.source||null,skill_uri:c.skill_uri||null,relation:c.relation||null}));const weighted=scores.reduce((a,x)=>a+x.score*x.weight,0)/Math.max(0.01,scores.reduce((a,x)=>a+x.weight,0));const note=box.querySelector('[data-note="'+id+'"]').value.trim();const rr=await db.rpc('submit_professional_evaluation',{p_match_request_id:id,p_evaluatee_profile_id:r.evaluatee,p_overall_rating:Number(weighted.toFixed(2)),p_criteria_scores:{framework:m.framework||'ESCO-aligned',framework_version:m.version||2,isco_group:m.isco_group||null,occupation_uri:m.occupation_uri||null,occupation_label:m.occupation_label||r.vacancy.title||null,criteria:scores,weighted_overall:Number(weighted.toFixed(2))},p_occupation_uri:m.occupation_uri||null,p_occupation_label:m.occupation_label||r.vacancy.title||null,p_note:note||null});if(rr.error){toast('تعذر إرسال التقييم الآن.');console.warn(rr.error);return;}toast('⭐ تم حفظ التقييم المهني الموزون بنجاح.');render();});
}
