...
(function () {
    ...
    madkhalRealJobsLoading = (async function () {
      try {
        ...
        const { data, error } = await supabaseClient
          .from("jobs")
          .select("id,source,source_job_id,title,company,description,location,country,job_type,category,salary_min,salary_max,salary_currency,source_url,canonical_url,posted_at,updated_at,expires_at,status,geo_scope,geo_region,geo_class,geo_reason")
          .eq("status", "active")
        ...
        console.warn("Madkhal runtime jobs validity filter:", e);
  ...
  function addIdentityFields() {
    ...
    [["workerFirstName",ID_KEYS.first],["workerFatherName",ID_KEYS.father],["workerFamilyName",ID_KEYS.family],["workerMotherName",ID_KEYS.mother],["workerBirthDate",ID_KEYS.birth]].forEach(function (pair) {
  ...
  function startTenMinuteRefresh() {
    if (refreshStarted) return; refreshStarted = true;
    setInterval(async function () {
      try {
        await realJobs(true);
        if (document.getElementById("opportunitiesScreen")?.classList.contains("active")) { madkhalJobDisplayLimit = 60; renderCategories(); renderOpportunities(); }
  ...
  window.applyOpportunity = async function (id) {
    ...
    modal.innerHTML = '<div style="background:#fff;border-radius:22px;padding:18px;width:min(700px,100%);max-height:92vh;overflow:auto;direction:rtl"><h3>📩 بيانات المتقدم</h3><p style="color:#657070;font-size:13px;line-height:1.7">هذه البيانات تمنع الخلط بين المتقدمين الذين يحملون الاسم نفسه.</p>' +
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><button id="applyConfirm" class="primary-btn" type="button">تأكيد التقديم</button><button id="applyCancel" class="secondary-btn" type="button">إلغاء</button></div></div>';
    ...
    modal.querySelector("#applyConfirm").onclick=async function(){
      ...
      localStorage.setItem(ID_KEYS.first,first);localStorage.setItem(ID_KEYS.father,father);localStorage.setItem(ID_KEYS.family,family);localStorage.setItem(ID_KEYS.mother,mother);localStorage.setItem(ID_KEYS.birth,birth);
      const apps=getLocalArray(LOCAL_KEYS.applications); if(apps.some(function(a){return String(a.job_id)===String(opportunity.id);})){modal.remove();showToast("ℹ️ سبق أن سجلت طلب تقديم على هذه الفرصة.");return;}
  ...
  document.addEventListener("DOMContentLoaded",function(){addIdentityFields();wrapProfileSave();startTenMinuteRefresh();setTimeout(function(){realJobs(false).then(function(){renderCategories();renderOpportunities();});},200);});
})();
